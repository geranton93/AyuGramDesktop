...
void ShowMenu(
	...
	if (id) {
		...
		MarkAsReadMenu::AddChatListAction(
			...
			addAction);
		AyuFeatures::HiddenFolders::AddToggleAction(
			addAction,
			session->userId().bare,
			id);
...
void ShowFiltersListMenu(
	...
	const auto list = AyuFeatures::HiddenFolders::VisibleOnly(
		session->userId().bare,
	...
	const auto hasAll = ranges::find(
		fullList,
		FilterId(),
		&Data::ChatFilter::id) != end(fullList);
	...
	for (auto i = 0; i < list.size(); ++i) {
		...
		auto item = base::make_unique_q<Ui::Menu::Action>(
			...
			icon,
			icon);
		action->setEnabled(i < visiblePremiumFrom);
		if (!title.text.empty()) {
...
not_null<Ui::RpWidget*> AddChatFiltersTabsStrip(
	...
	if (trackActiveFilterAndUnreadAndReorder) {
		...
		const auto applyReorder = [=](
			...
			if (!session->user()->isPremium() && !list.empty()) {
				if (list[0].id() != FilterId()) {
					filters->moveAllToFront();
				}
			}
			const auto visibleList = visibleFilters();
			if (oldPosition < 0
				|| newPosition < 0
				|| oldPosition >= int(visibleList.size())
				|| newPosition >= int(visibleList.size())) {
				return;
			}
			...
			order = AyuFeatures::HiddenFolders::ReorderVisible(
				...
				newPosition,
				[=](FilterId id) {
					return AyuFeatures::HiddenFolders::IsHidden(accountId, id);
				});
	...
	const auto filterByIndex = [=](int index)
	...
	const auto rebuild = [=] {
		...
		if (list.empty()) {
			...
			state->lastFilterId.reset();
			if (!trackActiveFilterAndUnreadAndReorder
				|| controller->activeChatsFilterCurrent() != FilterId()) {
				choose(FilterId());
			}
			wrap->toggle(false, anim::type::instant);
			return;
		...
		if (list.size() <= 1 && !slider->width()) {
			const auto first = list.front().id();
			if (!state->lastFilterId || *state->lastFilterId != first) {
				state->lastFilterId = first;
				choose(first);
			}
			return;
		}
		...
		slider->fitWidthToSections();
		{
			...
			const auto hasAll = ranges::find(
				...
				FilterId(),
				&Data::ChatFilter::id) != end(fullList);
			const auto premiumFrom = (reorderAll || !hasAll ? 0 : 1)
				+ maxLimit;
			const auto visiblePremiumFrom =
				AyuFeatures::HiddenFolders::VisiblePremiumFrom(
					accountId,
					fullList,
					premiumFrom);
			slider->setLockedFrom((visiblePremiumFrom >= list.size())
				? 0
				: visiblePremiumFrom);
			...
			if (state->reorder) {
				...
				state->reorder->clearPinnedIntervals();
				if (!reorderAll) {
					if (!list.empty() && list.front().id() == FilterId()) {
						state->reorder->addPinnedInterval(0, 1);
					}
				}
				state->reorder->addPinnedInterval(
					visiblePremiumFrom,
					std::max(1, int(list.size()) - visiblePremiumFrom));
			}
		}
		if (trackActiveFilterAndUnreadAndReorder) {
			reassignUnreadValue();
		...
		if (trackActiveFilterAndUnreadAndReorder) {
			controller->activeChatsFilter(
				...
				const auto list = visibleFilters();
				const auto fallback = list.empty()
					? FilterId()
					: list.front().id();
				auto found = false;
				for (auto i = 0; i < list.size(); ++i) {
	...
	session->data().chatsFilters().visibilityChanged()
	| rpl::on_next(rebuild, wrap->lifetime());
